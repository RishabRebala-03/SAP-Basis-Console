...
interface BulkUser {
  ...
  roles: string;
  profiles: string;
  status: "valid" | "invalid" | "processed" | "failed";
  errorMessage: string;
}
...
function SystemSelector({
  ...
  return (
    ...
    >
      <Server
        size={15}
        style={{ color: F.primary, flexShrink: 0 }}
      />
...
const inputCls =
  "px-3 py-1.5 text-xs rounded outline-none w-full";
...
function useHistoryFilters(entries: AuditLog[]) {
  ...
  const [appliedFilters, setAppliedFilters] =
    useState<HistoryFilters | null>(null);

  const [hasSubmitted, setHasSubmitted] =
    useState(false);

  const set =
    (k: keyof HistoryFilters) => (v: string) =>
      setFilters((f) => ({
        ...f,
        [k]: v,
      }));
  ...
  const filtered = useMemo(() => {
    ...
    if (currentFilters.status) {
      r = r.filter(
        (e) =>
          e.status === currentFilters.status
      );
    }
    ...
    if (currentFilters.system) {
      r = r.filter(
        (e) =>
          e.system === currentFilters.system
      );
    }
    ...
    r.sort((a, b) => {
      const av =
        (a[
          currentFilters.sortField as keyof AuditLog
        ] as string) ?? "";
      ...
      return currentFilters.sortDir === "asc"
        ? av.localeCompare(bv)
        : bv.localeCompare(av);
    });
...
function SearchAutocomplete({
  ...
  return (
    ...
    >
      ...
      {open && (
        <div
          className="absolute top-full left-0 right-0 mt-1 rounded shadow-xl z-40 overflow-hidden"
          style={{
            background: F.white,
            border: `1px solid ${F.border}`,
            minWidth: "280px",
...
function HistoryFilterBar({
  ...
  total,
  shown,
}: ReturnType<typeof useHistoryFilters> & {
  total: number;
  shown: number;
}) {
  const toggleSort = (
  ...
  return (
    ...
    >
      <div
        className="px-4 py-3 grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-3 items-end"
        style={{
          borderBottom: `1px solid ${F.border}`,
        }}
      >
        <div className="lg:col-span-2">
          ...
          <SearchAutocomplete
            ...
            actions={actions}
            placeholder="Records, system, action…"
          />
        </div>
...
function HistoryTab() {
  ...
  const allEntries = auditLogs.filter(
    (l) => l.module === "Bulk User"
  );

  const fh =
    useHistoryFilters(allEntries);
    ...
    <div>
      ...
      {!fh.hasSubmitted ? (
        ...
        >
          ...
          <p className="text-xs">
            Apply your search and filter
            criteria above, then click{" "}
            <strong>Go</strong> to view
            history.
          </p>
        </div>
      ) : filtered.length === 0 ? (
        <div
          className="flex flex-col items-center justify-center py-12 gap-2 rounded"
          style={{
            background: F.white,
            border: `1px solid ${F.border}`,
            color: F.muted,
        ...
        >
          ...
          <p className="text-sm">
            No entries match the submitted
            filters.
          </p>
        ...
        >
          <div
            className="px-5 py-3 grid grid-cols-[1fr_1fr_1fr_1fr_1fr_auto] gap-4 text-xs font-medium"
            style={{
              borderBottom: `1px solid ${F.border}`,
              color: "white",
              background: "#25313f",
          ...
          {filtered.map((log, idx) => {
            ...
            return (
              <div
                key={log.id}
                className="px-5 py-4 grid grid-cols-[1fr_1fr_1fr_1fr_1fr_auto] gap-4 items-start"
                style={{
                  borderBottom:
                    idx <
                    filtered.length - 1
...
export function BulkUserCreation({
  ...
  const parseFile = async (
    ...
    try {
      const records =
        await bulkPreviewApi(file);

      const mapped: BulkUser[] =
        records.map((r: any) => ({
          row:
            r.row_num ??
            r.row ??
  ...
  const handleProcess =
    async () => {
      if (!selectedSystem) {
        alert(
          "Please select a target SAP system before processing."
        );
        return;
      }
      ...
      try {
        ...
        logAction({
          module: "Bulk User",
          action: "Bulk Import",
          targetObject: `${users.length} records`,
          system:
            targetSystemId,
          client:
            sys?.client ?? "100",
          ...
          changesAfter:
            processed > 0
              ? `${processed} users provisioned in ${targetSystemId}/${sys?.client || "100"}`
              : undefined,
        });
      } catch (err: any) {
        setProcessingProgress(100);
        ...
        logAction({
          ...
          details:
            err.message ||
            "Bulk creation API request failed.",
          errorCode:
            "BULK_PROCESS_ERROR",
        });
      }
  ...
  const downloadReport = () => {
    const rows = users.map(
      (u) =>
        `${u.row},${u.username},${u.lastName},${u.validFrom},${u.validTo},${u.roles},${u.status},${u.errorMessage}`
    );

    const csv = [
      "Row,Username,Last Name,Valid From,Valid To,Roles,Status,Error",
      ...rows,
    ].join("\n");
    ...
    <div>
      ...
      {activeTab ===
        ...
        <>
          ...
          {parseError && (
            <div
              className="mb-4 flex items-start gap-3 px-4 py-3 rounded"
              style={{
                background: "#fff2f2",
          ...
          {uploadState ===
            "parsing" && (
            <div
              className="rounded flex flex-col items-center justify-center gap-4 py-14"
              style={{
                background: F.white,
                border: `2px dashed ${F.border}`,
              }}
            >
              <div
          ...
          {(uploadState ===
            ...
            <div>
              ...
              {uploadState ===
                "done" && (
                <div
                  className="mb-4 flex items-center gap-3 px-4 py-3 rounded"
                  style={{
                    background:
                      "#f1fdf6",
